...
Route.patch('/api/register/:id', customUserController.update).as('user.patch')
...
Route.get('/favposts/search', webCustomPostController.searchFavPosts).as('fav.search')
...
Route.get('/posts/search', webCustomPostController.searchPosts).as('posts.search')
Route.get('/api/posts', customPostController.list).as('post.list')
Route.get('/posts/:id', webCustomPostController.show).as('posts.show')
Route.patch('/api/posts/:id', customPostController.update).as('post.update')
...
// fav posts routes
Route.patch('/api/posts/:id/fav-posts', customPostController.favPost).as('favpost.patch')
